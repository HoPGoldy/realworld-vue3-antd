...
/** 获取热门标签列表 */
export const fetchTagList = async function () {
    const data: { tags: string[] } = await axios.get('/tags');
    return data.tags;
}

/** 创建新的文章 */
export const createArticle = async function (article: ArticleContent) {
    const data: { article: Article } = await axios.post('/articles', { article });
    return data.article;
}

/** 获取指定文章 */
export const fetchArticle = async function (slug: Slug) {
    const data: { article: Article } = await axios.get(`/articles/${slug}`);
    return data.article;
}

/** 更新指定文章 */
export const updateArticle = async function (slug: Slug, article: ArticleContent) {
...
/** 喜欢文章 */
export const favoriteArticle = async function (slug: Slug) {
    const data: { article: Article } = await axios.post(`/articles/${slug}/favorite`);
    return data.article;
}

/** 取消喜欢文章 */
export const unfavoriteArticle = async function (slug: Slug) {
